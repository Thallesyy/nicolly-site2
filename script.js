...
function prevPhoto() {
  if (!currentAlbum) return;
  photoIndex = (photoIndex - 1 + currentAlbum.photos.length) % currentAlbum.photos.length;
  animateSwap('right');
}

function animateSwap(direction) {
  // direction: 'left' = indo pra próxima foto, 'right' = indo pra foto anterior
  frontImg.classList.remove('anim-in-left', 'anim-in-right');
  frontImg.classList.add(direction === 'left' ? 'anim-out-left' : 'anim-out-right');

  setTimeout(() => {
    renderPhoto();
    frontImg.classList.remove('anim-out-left', 'anim-out-right');
    frontImg.classList.add(direction === 'left' ? 'anim-in-right' : 'anim-in-left');
  }, 180);
}
...
// ===== CAIXA DE TEXTO (FIREBASE REALTIME DATABASE, com fallback local) =====
// Import feito de forma assíncrona/opcional: se o Firebase não carregar (bloqueador de
// anúncios, extensão de privacidade, sem internet, etc.) o resto do site continua
// funcionando normalmente — a caixa de texto só passa a salvar apenas neste aparelho.
let fbSaveText = null;

(async function initFirebaseTextBox() {
  try {
    ...
    fbSaveText = (value) => set(textoRef, value);
  } catch (err) {
    console.warn('[Firebase] não carregou, a caixa de texto vai salvar só neste aparelho:', err);
  }
})();

// Salva com debounce de 800ms — sempre localmente, e também no Firebase se disponível
function saveText() {
  if (!textBox) return;
  clearTimeout(saveTimeout);
  saveHint.innerHTML = 'Salvando...';
  saveTimeout = setTimeout(() => {
    try { localStorage.setItem(STORAGE_KEY, textBox.value); } catch (e) {}
    ...
    if (fbSaveText) {
      fbSaveText(textBox.value).then(markSaved).catch(() => {
        saveHint.innerHTML = '⚠️ Salvo só neste aparelho';
      });
    } else {
      markSaved();
    }
